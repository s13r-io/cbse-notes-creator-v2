...
## Step 2 — Convert Parsed Text to Structured Markdown

After extracting text with `lit parse`, use an LLM to convert the raw text into well-structured Markdown. Works for any content type — letters, reports, articles, manuals, CBSE notes, and more.

A ready-to-use prompt is provided in:

```
.agents/skills/liteparse/prompts/convert-to-markdown.md
```

### How to use it

1. Read the prompt file
2. Copy the **Base Prompt** section
3. Replace `{document_description}` with a brief description (e.g., "a formal business letter", "CBSE Class 10 Science Chapter 5")
4. **For CBSE notes only:** append the **CBSE Notes Add-on** section, plus the relevant subject section (Science, Math, Social Science, English, or Hindi)
5. Append the parsed text at the bottom where indicated
6. Send the complete prompt to your LLM and save the response as a `.md` file

### Example flow

```bash
# Step 1: Parse the PDF
lit parse document.pdf --format text -o parsed.txt

# Step 2: Send parsed.txt + prompt to LLM
#
# For a general document (letter, report, etc.):
#   - Read .agents/skills/liteparse/prompts/convert-to-markdown.md
#   - Use Base Prompt only
#   - Replace {document_description} with "a formal business letter from ABC Corp"
#   - Append contents of parsed.txt
#   - Save LLM response as output/letter.md
#
# For CBSE notes:
#   - Use Base Prompt + CBSE Notes Add-on + Science add-on
#   - Replace {document_description} with "CBSE Class 10 Science Chapter 5 – Light"
#   - Append contents of parsed.txt
#   - Save LLM response as output/class10-science-ch5.md
```
